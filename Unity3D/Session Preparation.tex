...
\def\workshoptitle{Introduction to Unity}
\def\workshopsubtitle{Preparation Notes for Teachers}
\def\workshopauthor{Nick Phillips}
...
%///% Begin editable content
    ...
    \item The workshop is intended for students age 15+ (years 9+).

    \item The content is intended to be learnt through individual work led by a teacher, using the worksheet as guide.

    \item The learning platform is Unity3D.

    \item There are a number of versions of Unity, we use the latest online version.

    \item Students do not need to have used Unity before.
		
		\item Students should have an interest in 3 dimensional modeling (e.g. for design work or for 3D games) and some understanding of coordinates/geometry in 3 dimensions.
...
\valuelist{6cm}{
    \item [Logging on] If you have enough PCs/Macs for the whole group, please ensure everyone is logged on before we begin the session.
    ...
    \item [Software Installation] Installation of the latest version of Unity3D is required.
    
    \item [Website Access] Please ensure your IT administrator has unblocked the Unity website and our Github site: https://github.com/MVSE-Outreach/ The easiest way to test this is to get a student (not a teacher) to open the websites and attempt to browse them. 
