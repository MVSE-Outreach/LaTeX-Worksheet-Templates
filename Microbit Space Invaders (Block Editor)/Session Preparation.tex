...
\def\workshoptitle{Microbit Space Invaders}
...
%///% Begin editable content
    ...
    \item The learning platform is the BBC Micro:bit, the popular BBC's new educational platform as supplied to year 7s across the country.

		\item All year 7s in the country are being given Micro:bits for free for the next three years. Therefore, this workshop is ideally suited to Years 7 and 8 (since year 7s already received their Microbits last year).2

    \item Students do not need to have used the Micro:bit before but any time they have spent using the online Block Editor is very helpful.
		
		\item We can supply Micro:bits if required but if students have their own that is ideal. Micro:bits cost £15 per kit online.
...
\valuelist{6cm}{
    ...
    \item [Software Installation] No additional software is required for this workshop. 
    
    \item [Website Access] Please ensure your IT administrator has unblocked the Micro:bit website and download of the hex files that the website produces (that are saved to the Micro:bit devices). Please also ensure our Github site is unblocked: https://github.com/MVSE-Outreach/ The easiest way to test all this is to get a student (not a teacher) to open the websites and attempt to browse them. 
