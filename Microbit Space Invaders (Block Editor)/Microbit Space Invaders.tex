...
\def\workshopsubtitle{Block Editor - 2 Hours}
...
%///% Begin editable content
    ...
    \item This workshop is intended to last 2 hours.
    ...
    \valuelist{1cm}{
    	...
    	\item [*] Basic mathematical programming: Addition, Subtraction, Max/Min
...
\actions {
	...
	\item Click and drag a \enquote{forever} block into the middle of the screen.
}
...
\goals {
	Click run (play button) to see a simulation of your program in the editor. You should see the bottom-left LED light up.
}
...
\actions {
	\item Click and drag a \enquote{less-than} block (\enquote{\textless} block) from the \enquote{Logic} tab to the connector on the right of the word \enquote{if}
...
\actions {
	...
	\item Click and drag a \enquote{max} block from the \enquote{Maths:More} tab to the right-hand connector of the \enquote{set} block
}
...
\goals {
	Download and save your code to your Micro:bit - you should now see the Defender move left and right on the bottom of the display when you tilt the Micro:bit left and right.
}
...
\goals {
	Download and save your game to your Micro:bit. You should now be able to control the defender position so that you can put it in a particular column by tilting left and right.
}
...
\actions {
    \item Inside the \enquote{Variables} tab, use the \enquote{Make a variable} button to create the variables shown in the screenshot below.
}

\screenshot[.45\linewidth-0.8cm]{New-variable.png}{Creating a new variable using the drop-down arrow}
...
\goals {
	Download and save your game to your Micro:bit game to your Microbit. When the device powers on, you should now see the defender moving as before and the three invaders (staying still) in the top-left corner. (We'll make them move in the next section!)
}


\section{Starting the game}

As soon as the Micro:bit switches on, the game begins. But, we want to be able to draw the letters \enquote{Go!} on the screen then start the game. And at the end of the game, we want to show the player their score.

To be able to do this, we need to make sure our invaders and the defender are only drawn while the game is being played, not before and after. But! The game can only be being played when at least one of our invaders is alive! 

So we can make sure our invaders and defenders are only drawn when the game is being played, by putting an if-block around our \enquote{plot} code. The condition of the if block will be whether any of the invaders are alive.
...
\actions {
	...
	\item Click and drag an \enquote{or} block from the \enquote{Logic} tab into the conditional part of the \enquote{if} block
    \item Click and drag another \enquote{or} block into the inside of the \enquote{or} block you created in the previous step.
    \item Click and drag \enquote{Invader1}, \enquote{Invader2} and \enquote{Invader3} blocks from the \enquote{Variables} tab into the \enquote{or} blocks
	\item Click and drag the \enquote{if} block (and all its contents) back into the \enquote{forever} loop
}

\screenshot[.9\linewidth-0.8cm]{Drawing-code-only-when-playing.png}{Only draw the player / invaders when playing the game}

\goals {
	Download and save your game to your Micro:bit. When the Micro:bit powers on, you should no-longer see the defender at bottom of the device.
}
...
\actions {
	\item Add blocks to your game's code so that it matches the screenshot on the next page.
}

\screenshot[.77\linewidth-0.8cm]{Game-logic.png}{Code for the game logic}
...
\goals {
	Compile and save your game to your Micro:bit. You should see the invaders move around the display and the defender move at the bottom of the display.
}
