...
\actions {
	...
	\item Look at the Micro:bit files in the file explorer
	\item Open the \enquote{Microbit.htm} file (e.g. by double clicking) which should take you to the Micro:bit website
	\item Click the \enquote{Create Code} button
	\item In the \enquote{Microsoft Block Editor} box, click the \enquote{New Project} link
	\item At the top of the page, rename your project to \enquote{Space Invaders}
}

\screenshot{Microbit-Plugged-In.jpg}{Micro:bit plugged into laptop using the supplied USB cable}
\screenshot{Microbit-Files.png}{Files on the Micro:bit - double click the .htm file to open the website}
\screenshot{Microbit-Website.png}{Create Code on Micro:bit website}
...
\notes {
	An \enquote{if} block is a decision block (often called a Conditional block). You test something, and if that test passes, then the code inside the block executes. Otherwise, the code inside the block doesn't execute and the programs goes to the block following the if block.
}

\notes {
	The \enquote{test} the if-block uses is called the \enquote{condition}. It is a logic equation. For example, \enquote{is five greater than four} is a condition (to which the answer is always \enquote{yes} - usually called \enquote{true}). If the condition is \enquote{true} the code inside the if block executes, otherwise the condition is \enquote{false} so the code inside doesn't execute.
}

\todo{If block with parts highlighted}
...
\todo{Incomplete section of code}
