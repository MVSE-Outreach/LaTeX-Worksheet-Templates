...
%///% Begin editable content
    ...
    \item This workshop is intended to last 1 to 2 hours (depending on how much practice you want to do).
    ...
    \item This workshop is not intended for people outside the MVSE Outreach programme. It is aimed only at teaching what you need to be able to create new workshops for MVSE Outreach (see next point).
...
\itemlist {
    ...
    \item "Giant idiotic train wreck" : when it breaks
}


\subsection{I've heard a lot about the command line...}

Fear not - you do not need to use the command line to use Git. Honestly, some very good Git User Interfaces exist and they're often better than using command line (because very few people can use the Git command line without making regular mistakes or using less efficient commands than what the GUIs use). Command line is most useful if you want to automate stuff or are seriously into using the CLI for everything.
...
\actions {
    \item Head to \href{https://github.com/}{GitHub.com - https://github.com/} and sign up (or log in if you already have an account). 
}

\notes {
    If you're going to be working with the University of Bristol Outreach team, please sign up using your Bristol Uni email address (you'll also get access to student perks and discounts this way).
}

\goals {
    You have hopefully now signed up and verified your account.
}

\subsection{Joining the MVSE Outreach group}

\actions {
    \item Email Caroline Higgins (or speak to someone already on the group) to be granted access. 
}

\notes {
    Please remember to provide the email address you signed up with (which should have been your Bristol Uni email) - it needs to be exactly the same, not one of the many possible variants. 
}

\notes {
    You can find who's (publicly visible) in the group here: \href{https://github.com/MVSE-Outreach/people}{MVSE Outreach People - https://github.com/orgs/MVSE-Outreach/people}. By default, membership is private - it's up to you if you want to be publicly visible.
}

\goals {
    You have hopefully now joined the MVSE Outreach group.
}
...
\goals {
    You have hopefully now created your first repository on Github!
}
...
\actions {
    ...
    \item In a new tab, open the following file online (which also happens to be on GitHub): \href{https://github.com/github/gitignore/blob/master/TeX.gitignore}{Example LaTeX .gitignore : https://github.com/github/gitignore/blob/master/TeX.gitignore}
    \item Copy the contents of the file you just opened to the end of your existing .gitignore file. 
}

\notes {
    This new .gitignore will ignore any temporary files created by using LaTeX - you are expected to write worksheets for your workshop using the (very simple and easy to use) LaTeX template found in \href{https://github.com/MVSE-Outreach/LaTeX-Worksheet-Templates/}{https://github.com/MVSE-Outreach/LaTeX-Worksheet-Templates/}
}

\notes {
    You can edit your .gitignore at any time to exclude temporary files from commits. This is a good idea as it reduces clutter from the repository, reduces the size of the repo and prevents endless merge conflicts of binary files. Remember to ignore files before you commit them. Once a file is committed, it is tricky to get Git to ignore it again afterwards.
}

\actions {
    \item The example .gitignore doesn't ignore PDF files, which is a problem. So, at the end of the file, add the following: \texttt{*.pdf}\par
    This will ignore all .pdf files within your repository.
    \item Click the green \enquote{Commit changes} button
}

\goals {
    You have hopefully now edited your first file and committed your first change using Git!
}
\goals {
    Your .gitignore should be set up to ignore LaTeX temporary files and .PDF files.
}

\subsubsection{Read-Mes}

The Read Me (README.md) file is a Markdown formatted text file which, by default, is displayed beneath the list of files for your repo online. 

\actions {
    \item Go to the list of files in your repository online
    \item Click on the READ-ME file
    \item Open the editor for the READ-ME file (by clicking the same pencil button top-right as you did for .gitignore)
    \item Add a description of what your repository will contain / what your project is for to the READ-ME
    \item Add your name to the page somewhere - claim it as your own!
    \item Commit the change using the button at the bottom of the page
}

\goals {
    When viewing the home page of your repository, the default page (which is the READ-ME) should now include the paragraph of description you just added.
}
...
\goals {
    You should have successfully installed GitKraken.
}

\goals {
    You should have successfully configured GitKraken.
}

\goals {
    You should have successfully linked GitKraken to your GitHub account.
}
...
\goals {
    Hopefully you have now managed to clone a repo for the first time.
}
...
\group {
    So to make a new commit (after editing a file):
...
\actions {
    \item Try editing several files and making new commits
    \item Keep editing (even just random stuff) and making commits until you're satisfied you can remember all the steps. Committing is the most common and frequent task performed when using Git.
}

\goals {
    Done! You should now have pulled changes, edited a file (e.g. README.md), committed the changes and pushed the new commit(s).
}
